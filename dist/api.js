async function useTool(req, context) {
    ...
    const itemx = new ItemsService('automations', {
    ...
    let datax;
    try {
        datax = await itemx.readByQuery({ fields: ['*', 'source.*'], filter: { 'title': { '_eq': requestedTool } } });
    } catch (e) { }
    ...
    context.data.$tool = {};
    context.data.prevResponses = req.body.prevResponses ?? [];
    context.data.$tool.body = recursiveReplace(tool.body);
    ...
    if (req.body.finalReq == false) {
        context.data.prevResponses.push(context.data.apiResponse);
        context.data.prevResponses = [];
    }
...
const hooks = [];const endpoints = [{name:'sources-endpoint',config:e0}];const operations = [];
